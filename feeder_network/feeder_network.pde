...
void sujith()
{
    for (int i = 0; i < cols; i++) 
    {
      for (int j = 0; j < rows; j++) 
      {
         if (mouseX > ((i+1)*30-(myArray[i][j]/4)) && mouseX < ((i+1)*30+(myArray[i][j]/4)) &&  mouseY > ((j+1)*30-myArray[i][j]/4) && mouseY < ((j+1)*30+(myArray[i][j]/4)) ) 
        { if(mousePressed && (mouseButton == RIGHT)) {dubey [i][j]=2;}
